...
// --- FIXED: searchManga with case-insensitive filtering ---
async function searchManga() {
  ...
  try {
    ...
    if (activeGenreFilters.size > 0) {
      console.log('[app.js] Applying active genre filters to search results (case-insensitive):', activeGenreFilters);
      filteredItems = items.filter(manga => {
        ...
        const normalizedGenres = manga.genres.map(g => String(g).replace(/^genre\s*[:\-\s]*/i, '').trim());
        // Perform case-insensitive check: Does any manga genre (lowercased) match any filter (lowercased)?
        return normalizedGenres.some(mangaGenre =>
            Array.from(activeGenreFilters).some(filterGenre => filterGenre.toLowerCase() === mangaGenre.toLowerCase())
        );
      });
    }
...
// --- FIXED: Prevent background scroll ---
...
// --- FIXED: Restore background scroll ---
...
function updateGenreButtonStates() {
  ...
  // Update active filters display (Note: 'active-filters' ID might not exist in provided HTML)
...
// --- FIXED: Apply genre filters to manga list with case-insensitive matching ---

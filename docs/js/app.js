...
// --- State for Search Modal Filtering ---
...
async function openDetailsModal(mangaId, fallbackData) {
  ...
  if (genresContainer && mangaData.genres && Array.isArray(mangaData.genres) && mangaData.genres.length > 0) {
      ...
      // sanitize genre strings to remove redundant prefixes like "Genre " or repeated words
      const used = new Set();
      mangaData.genres.forEach(genreRaw => {
          if (!genreRaw) return;
          let genre = String(genreRaw).trim();
          // Remove leading word "genre" if present (case-insensitive)
          ...
          genreSpan.className = 'genre-pill'; // new pill class (CSS will give it a clean look)
  ...
  if (chapterListContainer) {
      if (mangaData.chapters && Array.isArray(mangaData.chapters) && mangaData.chapters.length > 0) {
          ...
          sortedChapters.forEach(ch => {
              ...
              `;
              // Make the chapter row clickable to quickly open the reader popup
              chapterDiv.style.cursor = 'pointer';
              chapterDiv.onclick = () => {
                // Try to load the chapter in the popup reader
...
// --- MODIFIED: Unified search + filter function for search modal ---
async function populateSearchResultsFromFilters() {
  ...
  try {
    ...
    box.innerHTML = '<p class="muted">Loading results...</p>'; // Show loading
    let items = [];
    if (q) {
      items = await searchTitles(q);
    } else {
      // If no search term, show all items (unfiltered by main view filters)
      items = [...allMangaItems];
    }

    // Apply filters ONLY if search filter is active and there are filters set
    if (isSearchFilterActive && searchActiveGenreFilters.size > 0) {
      ...
      items = items.filter(manga => {
        ...
        // Sanitize and normalize manga genres for comparison (KEY FIX: .toLowerCase())
        const normalizedGenres = manga.genres.map(g => String(g).replace(/^genre\s*[:\-\s]*/i, '').trim().toLowerCase()).filter(g => g);
        // Check if any of the manga's genres match the active search filters
        return normalizedGenres.some(genre => searchActiveGenreFilters.has(genre));
      });
    }
    ...
    // Always hide the load more button in search results
...
// --- MODIFIED: searchManga now delegates to the unified function ---
...
// --- MODIFIED: Prevent background scroll ---
function openSearchModal() {
  ...
  if (m) {
    ...
    setTimeout(() => {
        ...
        // Populate search results immediately (empty query -> show all or filtered)
...
// --- MODIFIED: Restore background scroll ---
...
async function loadMoreTrending() {
  ...
  try {
    ...
    applyGenreFilters(); // Re-apply filters if any to main view
...
async function loadMoreUpdates() {
  ...
  try {
    ...
    allMangaItems = [...trendingItems, ...featuredItems];
    renderUpdates(featuredItems); // Render updated list (unfiltered)
    if (data.pagination && data.pagination.length > 0) {
      const totalPages = data.pagination[data.pagination.length - 1];
...
/* ---- Genre Filter Functions (now with modal + checklist) ---- */
...
function createGenreCheckboxes() {
  ...
  const sorted = Array.from(allGenres).sort((a,b)=> a.localeCompare(b));
  if (sorted.length === 0) {
  ...
  sorted.forEach(genre => {
    ...
    cb.onchange = (e) => {
      ...
      if (isSearchOpen) {
          // If search modal is open, modify search filters (KEY FIX: .toLowerCase())
          const filterKey = e.target.value.toLowerCase();
          if (e.target.checked) searchActiveGenreFilters.add(filterKey);
          else searchActiveGenreFilters.delete(filterKey);
      } else {
          // If main view, modify main filters
          if (e.target.checked) activeGenreFilters.add(e.target.value);
          else activeGenreFilters.delete(e.target.value);
      }
...
// Update visual state of genre buttons / checkboxes and the active filters display
function updateGenreButtonStates() {
  // Determine context (search modal open or not)
  const searchModal = document.getElementById('search-modal');
  const isSearchOpen = searchModal && window.getComputedStyle(searchModal).display !== 'none';

  // Sync checkboxes in modal based on context
  ...
  checkboxes.forEach(cb => {
    if (isSearchOpen) {
        ...
        cb.checked = searchActiveGenreFilters.has(cb.value.toLowerCase());
    } else {
  ...
  // Update active filters display (if element exists - might be from previous version)
  const activeFiltersEl = document.getElementById('active-filters');
  if (activeFiltersEl) {
    if (isSearchOpen && searchActiveGenreFilters.size > 0) {
      activeFiltersEl.textContent = `Active filters: ${Array.from(searchActiveGenreFilters).join(', ')}`;
...
// Apply genre filters to main manga list (Trending section)
...
// Called when user presses Apply inside filter modal
...
async function init() {
  try {
    ...
    // Create (hidden) genre UI / checkboxes
    createGenreCheckboxes();
    updateGenreButtonStates();
  } catch (e) {
